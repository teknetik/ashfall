...
async function sampleCanvas(page) {
  ...
  // A locator screenshot can include HUD elements layered over the canvas. Hide them for sampling.
  const buffer = await locator.screenshot({ style: 'body * { visibility: hidden !important; } canvas { visibility: visible !important; }' });
...
async function main() {
  ...
  if (!result.ok || result.diagnostics?.state === 'error' || result.diagnostics?.error || consoleErrors.length > 0 || pageErrors.length > 0) {
    process.exit(1);
  }
}
