 ...
 public class GameSession:MonoBehaviour
 {
  ...
  void Update()
  {
   ...
   if(State==CityState.Play)
   {
    ...
    if(!visitedHill&&new Vector2(p.x,p.z).magnitude<7&&p.y>1){visitedHill=true;Changed?.Invoke();}
    bool inside=Mathf.Abs(p.x)<5.4f&&Mathf.Abs(p.z-36.1f)<2.6f&&p.y>.3f;
    if(inside&&!ringInside)Notify("Destination offline. The far ring has gone quiet.","Ring Gate");ringInside=inside;
   }
   if(State==CityState.Grid&&GridProgress<1){GridProgress=Mathf.Min(1,GridProgress+Time.deltaTime/catalog.transitionSeconds);Changed?.Invoke();}
  ...
  bool NearLattice=>Vector3.Distance(player.transform.position,new Vector3(0,.5f,-39))<3.15f;
  bool NearRing=>Mathf.Abs(player.transform.position.x)<5.4f&&Mathf.Abs(player.transform.position.z-36.1f)<2.8f;
