...
namespace AthenHill
{
 public class AthenDebugBridge : MonoBehaviour
 {
  ...
  public void View(string name)
  {
   ...
   var c=Object.FindObjectsByType<Camera>().FirstOrDefault(x=>x.name==name);
   if(!c)throw new System.ArgumentException("Unknown camera: "+name);
   follow.FixedView=true;
   bool review=name=="character"||name=="portrait";var rotation=review?player.visual.rotation:Quaternion.identity;
   var position=review?player.transform.position+rotation*(c.transform.position-player.spawn.position):c.transform.position;
   follow.transform.SetPositionAndRotation(position,rotation*c.transform.rotation);follow.GetComponent<Camera>().fieldOfView=c.fieldOfView;selectedCamera=name;
