...
{
 public static class ImportDistrictAssets
 {
  public const string Folder="Assets/AthenHill/Art/Imported/Meshy/District";
  public const string Prefabs="Assets/AthenHill/Prefabs/District";
  public static readonly string[] Names={"water","tools","salvage","finery","field","repairs","thread","gate"};
  static readonly Vector3[] Sizes={new(7.6f,10.7f,8.8f),new(7.6f,7.7f,8.8f),new(7.6f,9.7f,8.8f),new(7.6f,10.7f,8.8f),new(7.6f,10.7f,8.8f),new(7.6f,7.7f,8.8f),new(7.6f,9.7f,8.8f),new(11.5f,8.8f,3.4f)};
  ...
  public static void Prepare()
  {
   ...
   {
    var dest=Folder+"/"+name;Directory.CreateDirectory(dest);var source="../../meshy/district-20260908/"+name;
    File.Copy(source+"/"+name+".fbx",dest+"/"+name+".fbx",true);
    foreach(var file in Directory.GetFiles(source+"/"+name+"_textures","*.png"))File.Copy(file,dest+"/"+Path.GetFileName(file),true);
   }
   ...
   for(int i=0;i<Names.Length;i++)
   {
    ...
    var model=AssetDatabase.LoadAssetAtPath<GameObject>(path);var root=new GameObject(name);var fit=new GameObject("Fitted visual").transform;fit.SetParent(root.transform,false);var visual=(GameObject)PrefabUtility.InstantiatePrefab(model);visual.transform.SetParent(fit,false);visual.name="Meshy visual";
    // Meshy authored Field Supply's storefront on +X; all runtime prefabs face +Z.
    if(name=="field")visual.transform.localRotation=Quaternion.Euler(0,-90,0)*visual.transform.localRotation;
    var source=BoundsOf(root);
    // The parcel is a maximum envelope, not a box to stretch the building into.
    // Preserve the authored proportions, especially door heights and roof profiles.
    float scale=Mathf.Min(Sizes[i].x/source.size.x,Sizes[i].y/source.size.y,Sizes[i].z/source.size.z);
    fit.localScale=Vector3.one*scale;
    var b=BoundsOf(root);fit.localPosition-=new Vector3(b.center.x,b.min.y,b.center.z);
   ...
   AssetDatabase.SaveAssets();Directory.CreateDirectory("../evidence/district/20260908/import");File.WriteAllText("../evidence/district/20260908/import/models.json",JsonConvert.SerializeObject(reports,Formatting.Indented));
   Preview();
  }
  ...
  public static Material MaterialFor(string name)
  {
   ...
   var path=Folder+"/"+name+"/"+name+".mat";var mat=AssetDatabase.LoadAssetAtPath<Material>(path);if(!mat){mat=new Material(Shader.Find("Universal Render Pipeline/Lit")){name="District_"+name,enableInstancing=true};AssetDatabase.CreateAsset(mat,path);}
  ...
  public static void Preview()
  {
   ...
   foreach(var name in Names)
   {
    ...
    {
     var c=ImportBaseline.Camera("preview",b.center+new Vector3(d*.65f,d*.25f,(side==0?1:-1)*d),b.center,42);c.clearFlags=CameraClearFlags.SolidColor;c.backgroundColor=new Color(.36f,.36f,.34f);PortDiagnostics.Capture("preview");PortDiagnostics.Capture("preview");File.Copy("Captures/Fixed/preview.png","../evidence/district/20260908/import/"+name+(side==0?"-plusz":"-minusz")+".png",true);UnityEngine.Object.DestroyImmediate(c.gameObject);
    }
