 ...
 public static class ImportSalvageAssets
 {
  ...
  public static void Prepare()
  {
   ...
   for(int i=0;i<Names.Length;i++)
   {
    ...
    var source=BoundsOf(root);visual.transform.localScale*=Mathf.Min(Sizes[i].x/source.size.x,Sizes[i].y/source.size.y,Sizes[i].z/source.size.z); // Preserve authored proportions inside the parcel.
  ...
  static Texture2D Texture(string name,string file,bool srgb,bool normal=false,bool readable=false)
  {
   ...
   var i=(TextureImporter)AssetImporter.GetAtPath(path);i.textureType=normal?TextureImporterType.NormalMap:TextureImporterType.Default;i.sRGBTexture=srgb;i.GetSourceTextureWidthAndHeight(out int width,out int height);i.maxTextureSize=Mathf.NextPowerOfTwo(Mathf.Max(width,height));i.mipmapEnabled=true;i.anisoLevel=8;i.isReadable=readable;i.textureCompression=readable?TextureImporterCompression.Uncompressed:TextureImporterCompression.CompressedHQ;i.SaveAndReimport();return AssetDatabase.LoadAssetAtPath<Texture2D>(path);
  }
  static Material MaterialFor(string name)
  {
   ...
   mat.SetTexture("_BaseMap",albedo);mat.SetColor("_BaseColor",Color.white);mat.SetTexture("_BumpMap",normal);mat.SetFloat("_BumpScale",1);mat.EnableKeyword("_NORMALMAP");mat.SetTexture("_MetallicGlossMap",Texture(name,"metallic_smoothness",false));mat.SetFloat("_Metallic",1);mat.SetFloat("_Smoothness",1);mat.EnableKeyword("_METALLICSPECGLOSSMAP");mat.SetFloat("_Cull",2);EditorUtility.SetDirty(mat);return mat;
  }
